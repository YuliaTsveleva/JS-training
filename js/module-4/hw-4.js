...
// Если в свойстве pizzas нет пиццы с названием 
// из параметра pizzaName, метод order должен 
// возвращать результат вызова колбэка onError, 
// передавая ему аргументом строку 'В ассортименте 
// нет пиццы с названием < имя пиццы >.'
...
// доставки еды.
// Выполни рефакторинг методов объекта customer, 
// расставив отсутствующие this при обращении к 
// свойствам объекта.

// После объявления объекта мы добавили вызовы 
// методов в той последовательности, в которой 
// твой код будут проверять тесты.
...
// чтобы она возвращала строку в формате 
// 'Готовим <блюдо> для <почта>. Ваш заказ <позиция>-й 
// в очереди.' Позиция это значение параметра position 
...
// Используй call для вызова функции в контексте 
// одного объекта - заказа.
// Используй this в теле функции для доступа к 
// свойствам объекта - заказа в контексте которого 
// она была вызывана.
// Дополни код так, чтобы в переменной messages 
// получился массив сообщений о статусе заказов 
...
// сообщения для ресторанов. Дополни код так, чтобы 
// в переменных pizzaPalaceComposer и burgerShackComposer 
...
// Функция logAndInvokeAction(email, action) ожидает 
// почту и действие которое нужно выполнить - 
